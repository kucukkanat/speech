...
description: "On-device, voice-cloning text-to-speech for the browser. Runs Chatterbox on WebGPU in a Web Worker: no server, no API key, nothing leaves the device."
icon: audio-lines
---

`@kucukkanat/tts` · [source](https://github.com/kucukkanat/speech/tree/main/packages/tts)
...
:::tip
The [voice cloning guide](/guides/voice-cloning) walks through recording, cloning and speaking end to end, with a live
demo.
:::
...
[`@kucukkanat/voices`](/packages/voices) and speak with the saved record.

Try it: record yourself, then press speak.
...
[`@kucukkanat/voices`](/packages/voices), a ready-made voice library:
...
[`@kucukkanat/speech-react`](/packages/react)):
...
Step-by-step setups, including webpack and Next.js, are in the [bundler guide](/guides/bundlers).
